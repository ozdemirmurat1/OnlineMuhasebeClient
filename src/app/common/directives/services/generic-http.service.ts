...
export class GenericHttpService {
  ...
  lloginPost<T>(api:string,model:any,callBack:(res:T)=> void,authorize:boolean=true,diffApi:boolean=false){
    //this.getToken()
    if(!this.isLoading)
        this.store.dispatch(changeLoading());
    this._http.post<T>(`${this.setApi(diffApi,api)}`,model,this.setOptions(authorize)).subscribe({
  ...
  getToken(){
    ...
    if(this.token!="" && this.token!=undefined && this.token!=null){
      ...
      if(time>decoded.exp){
        if(refreshToken){
          let model :{userId:string, refreshToken:string, companyId:string}={userId:this.loginResponse.userId,refreshToken:this.loginResponse.token.refreshToken,companyId:this.loginResponse.company.companyId};
